...
class ImportTransactionsService {
  async execute(filePath: string): Promise<Transaction[]> {
    ...
    const categoriesTitles = existCategories.map((cat: Category) => cat.title);

    const addCategoryTitles = categories
      .filter(cat => !categoriesTitles.includes(cat))
    ...
    const finalCategories = [...newCategory, ...existCategories];
